...
void displayBoard(int size, Cell board[][size], bool displayMines)
{
    int r, c;
    printf("\n\n   ");
    for(c=1; c<=size; c++){ //print col numbers along the top
        printf("%3d", c);
    }
    printf("\n");
    for(r=0; r<size; r++){
        printf("%3d", (r+1)); //print row numbers
        for(c=0; c<size; c++){
            if(displayMines == 1){
                ...
                if(board[r][c].is_mine == 1 && board[r][c].mines != 100){
                    printf("  ?");
                }
                else if(board[r][c].is_mine == 1 && board[r][c].mines == 100){
                    printf("  *");
                }
                else if(board[r][c].visible == 1){
                    printf("%3d", board[r][c].mines);
...
Status selectCell(int row, int col, int size, Cell board[][size])
{
    int i = size*size-nbrOfMines(size, board);
    if(board[row][col].is_mine == 1){
        board[row][col].mines = 100;
        return LOST;
    }
    else if(board[row][col].is_mine == 0){
        if(nbrVisibleCells(size,board) == i){
            return WON;
        }
        else{
...
void setImmediateNeighborCellsVisible(int row, int col, int size, Cell board[][size])
{
    ...
    if(board[row][col].mines == 0){
        board[row][col].visible = 1;

        if ((row-1)>0 && (row-1)<=size){ //check above
            board[row-1][col].visible = 1;
        }
        if ((row+1)>0 && (row+1)<=size){ //check below
            board[row+1][col].visible = 1;
        }
        if ((col-1)>0 && (col-1)<=size){ //check left
            board[row][col-1].visible = 1;
        }
        if ((col+1)>0 && (col+1)<=size){ //check right
            board[row][col+1].visible = 1;
        }
    }
    else{
...
void setAllNeighborCellsVisible(int row, int col, int size, Cell board[][size])
{
    ...
    board[row][col].visible = 1;
    if (board[row][col].mines == 0) {
        setAllNeighborCellsVisible((row-1), col, size, board); //check above
        setAllNeighborCellsVisible(row, (col+1), size, board); //check right
        setAllNeighborCellsVisible(row, (col-1), size, board); //check left
        setAllNeighborCellsVisible((row+1), col, size, board); //check below
    }
}
